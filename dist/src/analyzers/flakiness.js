...
import { applyScore, createCategory, exists, findConfig, findFiles, readFile, } from '../utils/helpers.js';
...
export async function analyzeFlakiness(targetDir) {
    ...
    const anyTestMatches = (re) => sampleTexts.some((t) => re.test(t));
    const offenders = (re) => sample.filter((f, i) => re.test(sampleTexts[i] || '')).slice(0, 10);
    ...
    const forbidOnlyConfigured = /forbidOnly\s*:\s*(true|process\.env\.CI)/i.test(cfgText) ||
    ...
    const uploadsHtmlReport = workflowTexts.some((t) => /actions\/upload-artifact/i.test(t) && /playwright-report/i.test(t));
    ...
    const rules = [
        ...
        // ---- Anti-patterns in tests (artifact list implied by title; keep structure unchanged)
        {
            ...
            suggestion: 'Replace sleeps with explicit waits, e.g., `await expect(locator).toBeVisible()`.',
        ...
        },
        {
            ...
            file: offenders(/describe\.configure\(\s*{[^}]*mode\s*:\s*['"`]serial['"`]/)[0] ||
                sample[0] ||
                '',
        },
        // ---- HTML report persisted (folder + CI upload artifact)
        {
            ...
            suggestion: 'Upload `playwright-report` (or your HTML report path) as an artifact via actions/upload-artifact.',
