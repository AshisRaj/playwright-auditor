...
import { applyScore, createCategory, exists, fileExistsAny, inDeps, ok, readFile, readJsonSafe, readTsconfig, walkFiles, } from '../utils/helpers.js';
/** Find first N files with an anti-pattern (e.g., waitForTimeout). */
async function findAntiPatternFiles(targetDir, re, roots = ['src', 'tests', 'test', 'e2e', '__tests__', 'scripts'], limitArtifacts = 5) {
    const out = [];
...
export async function analyzeBestPractices(targetDir) {
    ...
    const eslintrcPath = (await fileExistsAny(targetDir, [
        '.eslintrc.json',
        '.eslintrc.js',
        '.eslintrc.cjs',
        'eslint.config.js',
    ...
    const prettierPath = (await fileExistsAny(targetDir, [
        '.prettierrc',
        '.prettierrc.json',
        '.prettierrc.js',
        'prettier.config.js',
    ...
    const lintStagedConfigured = !!pkg['lint-staged'] ||
    ...
    const prTemplatePath = (await fileExistsAny(targetDir, ['.github/pull_request_template.md'])) ||
    ...
    const formatRunsPrettier = /\bprettier\b/.test(formatScript) || /\bprettier\b/.test(formatFixScript);
    ...
    const checks = [
        ...
        },
        {
            ...
            file: pkgPath,
            suggestion: 'Ensure your format scripts call Prettier, e.g., "prettier --check ." and "prettier --write ."',
        },
        {
            id: 'lint-script',
