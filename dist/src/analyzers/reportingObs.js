...
import { addFinding, applyScore, createCategory, exists, findConfig, findFiles, makeId, readFile, readJson, } from '../utils/helpers.js';
...
function extractReporterBlock(src) {
...
function hasReporter(reporterBlock, name) {
    ...
    if (arrayContainsBare.test(reporterBlock))
        return true;
    // 3) tuple with options, e.g. ['json', { outputFile: ... }]
    const tuple = new RegExp(`\\[\\s*(['"])${name}\\1\\s*,`, 'i');
    if (tuple.test(reporterBlock))
        return true;
    // 4) common package names
    if (name === 'allure') {
        if (/\b(allure-playwright|@shelex\/allure-playwright)\b/i.test(reporterBlock))
            return true;
    }
    if (name === 'monocart') {
        if (/\bmonocart-reporter\b/i.test(reporterBlock))
            return true;
    }
    return false;
}
export async function analyzeReportingObs(targetDir) {
    ...
    const traceConfigured = /\btrace\s*:\s*['"`](on|retain-on-failure|on-first-retry)['"`]/i.test(pwText) ||
        /\btrace\s*:\s*\w+/i.test(pwText); // allow variables like trace: isCI ? 'on' : 'off'
    const screenshotConfigured = /\bscreenshot\s*:\s*['"`](on|only-on-failure)['"`]/i.test(pwText) ||
        /\bscreenshot\s*:\s*\w+/i.test(pwText);
    const videoConfigured = /\bvideo\s*:\s*['"`](on|retain-on-failure)['"`]/i.test(pwText) ||
    ...
    const uploadsHtmlReport = workflowTexts.some((t) => /upload-artifact[\s\S]*playwright-report/i.test(t));
    ...
    const uploadsAllure = workflowTexts.some((t) => /upload-artifact[\s\S]*allure-(results|report)/i.test(t));
    // Dependencies (for Allure/Monocart)
    const pkgPath = path.join(targetDir, 'package.json');
    const pkg = (await readJson(pkgPath)) || {};
    const allDeps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
    const hasAllurePkg = 'allure-playwright' in allDeps ||
    ...
    addFinding(cat, {
        ...
        status: (hasAllureReporter || hasAllurePkg),
        severity: 'low',
        message: hasAllureReporter || hasAllurePkg
            ? 'Allure reporter dependency/config detected'
            : 'Not configured',
        suggestion: 'If you use Allure, install allure-playwright and wire the reporter; upload allure-results in CI.',
        artifacts: [
            pwConfigPath || '',
    ...
    addFinding(cat, {
        ...
        status: (hasMonocartReporter || hasMonocartPkg),
        severity: 'info',
        message: hasMonocartReporter || hasMonocartPkg ? 'Monocart reporter detected' : 'Not configured',
    ...
    addFinding(cat, {
        ...
        status: (hasPlaywrightReportDir || uploadsHtmlReport),
        severity: 'low',
        message: hasPlaywrightReportDir || uploadsHtmlReport
    ...
    addFinding(cat, {
        ...
        status: (hasTraceDir || uploadsTraces),
        severity: 'info',
        message: hasTraceDir || uploadsTraces
    ...
    addFinding(cat, {
        ...
        status: (hasScreenshotsDir || uploadsScreens),
        severity: 'info',
        message: hasScreenshotsDir || uploadsScreens
    ...
    addFinding(cat, {
        ...
        status: (hasAllureResultsDir || uploadsAllure),
        severity: 'info',
        message: hasAllureResultsDir || uploadsAllure
            ? 'Allure results are persisted'
            : 'No Allure results artifact detected',
        suggestion: 'Upload `allure-results/` in CI and publish the report (`allure generate` or action).',
    ...
    addFinding(cat, {
        ...
        status: (attachFiles.size > 0),
        severity: 'info',
        message: attachFiles.size > 0
            ? `Found attachments in ${attachFiles.size} file(s)`
            : 'No test.info().attach usage detected',
        suggestion: 'Use `test.info().attach(name, { body, contentType })` to include logs, HARs, or snapshots in reports.',
        artifacts: pick(attachFiles),
    });
    addFinding(cat, {
        id: makeId('Structured steps in tests'),
        title: 'Structured steps in tests',
        status: (stepFiles.size > 0),
        severity: 'info',
        message: stepFiles.size > 0
            ? `Found test.step in ${stepFiles.size} file(s)`
            : 'No test.step usage detected',
        suggestion: 'Wrap logical actions in `await test.step("description", async () => { ... })` for clearer reports.',
