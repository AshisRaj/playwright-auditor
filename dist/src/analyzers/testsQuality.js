...
export async function analyzeTestsQuality(targetDir) {
    const roots = ['tests', 'e2e', 'test', '__tests__', 'src/tests', 'src'].map((r) => path.join(targetDir, r));
    // discover test files
    let testFiles = [];
    ...
    const usesDescribe = new Set();
    const usesStep = new Set();
    const usesSoft = new Set();
    const usesFixtures = new Set(); // test.use/test.extend/base.extend
    const usesAwait = new Set();
    const antiWaitForTimeout = new Set();
    const pageWaitForSelector = new Set(); // prefer locator assertions
    const testOnly = new Set();
    const describeOnly = new Set();
    const testSkip = new Set();
    const testFixme = new Set();
    const snapshotFiles = new Set(); // expect(page).toHaveScreenshot / toMatchSnapshot
    const parallelDescribe = new Set(); // test.describe.configure({ mode: 'parallel' })
    const networkMocking = new Set(); // page.route/context.route
    const explicitTimeouts = new Set(); // test.setTimeout(...), test.slow()
    // regexes
    ...
    const reParallelDescribe = /\btest\.describe\.configure\s*\(\s*{[^}]*\bmode\s*:\s*['"]parallel['"]/;
    ...
    for (const f of take) {
        ...
        if (!s)
            continue;
        if (reDescribe.test(s))
            usesDescribe.add(f);
        if (reStep.test(s))
            usesStep.add(f);
        if (reSoft.test(s))
            usesSoft.add(f);
        if (reFixtures.test(s))
            usesFixtures.add(f);
        if (reAwait.test(s))
            usesAwait.add(f);
        if (reWaitForTimeout.test(s))
            antiWaitForTimeout.add(f);
        if (rePageWaitForSelector.test(s))
            pageWaitForSelector.add(f);
        if (reTestOnly.test(s))
            testOnly.add(f);
        if (reDescribeOnly.test(s))
            describeOnly.add(f);
        if (reTestSkip.test(s))
            testSkip.add(f);
        if (reTestFixme.test(s))
            testFixme.add(f);
        if (reScreenshot.test(s))
            snapshotFiles.add(f);
        if (reParallelDescribe.test(s))
            parallelDescribe.add(f);
        if (reRoute.test(s))
            networkMocking.add(f);
        if (reExplicitTimeouts.test(s))
            explicitTimeouts.add(f);
    }
    ...
    // 4) Avoid waitForTimeout anti-pattern
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Replace waitForTimeout with locator assertions (e.g., await expect(locator).toBeVisible()).',
    ...
    // 5) Prefer locator assertions over page.waitForSelector
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Prefer await expect(locator).toBeVisible()/toHaveText() etc. Locator assertions auto-wait and are more resilient.',
    ...
    // 6) Guard against focused tests (test.only / describe.only)
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Delete .only before pushing; enforce via a pre-commit/CI grep to block focused tests.',
    ...
    // 7) Skips / fixme usage check (informational; too many may hide issues)
    {
        ...
        addFinding(cat, {
            ...
            status: ok,
            severity: 'info',
            message: totalSkips === 0
                ? 'No test.skip/fixme usage detected'
                : `Found ${totalSkips} occurrence(s) of skip/fixme`,
            suggestion: 'Keep skip/fixme temporary; track issues and remove regularly to avoid masking failures.',
    ...
    // 8) Snapshot usage present (positive signal for UI stability)
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Use toHaveScreenshot()/toMatchSnapshot() for regressions where DOM/text checks are insufficient.',
    ...
    // 9) Parallelization hints
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Consider parallel mode for independent suites to reduce runtime; ensure test isolation first.',
    ...
    // 10) Network mocking present in tests (route usage)
    {
        ...
        addFinding(cat, {
            ...
            suggestion: 'Mock network for flaky/slow dependencies to make tests deterministic; prefer page.route for per-test scope.',
            artifacts: pick(networkMocking),
        });
    }
    // 11) Explicit timeouts/slow markers (reasonable use)
    {
        const ok = explicitTimeouts.size === 0 ||
        ...
        addFinding(cat, {
            ...
            status: ok,
            severity: 'low',
            message: explicitTimeouts.size === 0
                ? 'No test.setTimeout/test.slow detected'
                : `Explicit timeouts/slow in ${explicitTimeouts.size} file(s)`,
            suggestion: 'Use explicit timeouts sparingly; fix root causes instead. If needed, annotate justification in code review.',
