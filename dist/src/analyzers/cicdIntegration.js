import { applyScore, collectCiConfigs, createCategory, firstFileMatching, } from '../utils/helpers.js';
const cat = createCategory('cicd', 'CI/CD Integration');
export async function analyzeCICDIntegration(targetDir) {
    ...
    const nodeCacheFile = firstFileMatching(scan, /\b(cache\s*:\s*['"](npm|yarn|pnpm)['"]|actions\/cache@)/i);
    ...
    const uploadArtifactFile = firstFileMatching(scan, /\b(upload-artifact|store_artifacts|artifacts\s*:)/i);
    const artifactRetentionFile = firstFileMatching(scan, /\b(retention-days\s*:\s*\d+|artifacts\s*:[\s\S]*?\bexpire_in\s*:)/i);
    ...
    const shardInCmdFile = firstFileMatching(scan, /\bnpx\s+playwright\s+test\b[^\n]*--shard\s*=\s*\S+/i);
    ...
    const glMrTriggerFile = firstFileMatching(scan, /\bonly\s*:\s*\[?\s*merge_requests\s*\]?|rules\s*:\s*[\s\S]*\$CI_MERGE_REQUEST_IID/i);
    const prMrPass = !!ghPrTriggerFile || !!glMrTriggerFile;
    // Coverage upload (Codecov) or lcov upload step
    const coverageFile = firstFileMatching(scan, /\b(codecov\/codecov-action@|bash\s*<\(\s*curl.*codecov\.io|lcov|coverage\/lcov\.info)\b/i);
    const coveragePass = !!coverageFile;
    // GH concurrency cancel-in-progress
    const concurrencyFile = firstFileMatching(scan, /\bconcurrency\s*:\s*[\s\S]*\bcancel-in-progress\s*:\s*(true|yes)/i);
    const concurrencyPass = !!concurrencyFile;
    // xvfb/headless setup (Linux jobs often need this for video/screenshots)
    const xvfbFile = firstFileMatching(scan, /\bxvfb-run\b|xvfb\s+start|uses\s*:\s*GabrielBB\/xvfb-action@/i);
    ...
    const htmlReportFile = firstFileMatching(scan, /(playwright-report|html-report)[/'"]?\b.*\b(upload-artifact|store_artifacts|artifacts\s*:)/i);
    const tracesFile = firstFileMatching(scan, /(trace|traces|test-results)[/'"]?\b.*\b(upload-artifact|store_artifacts|artifacts\s*:)/i);
    ...
    const checks = [
        ...
        },
        {
            ...
            suggestion: 'Use actions/setup-node with `cache: npm|yarn|pnpm` or GitLab `cache:` with a key to speed up installs',
        ...
        },
        {
            ...
            suggestion: 'Enable actions/cache (GH) or the `cache:` key (GL) to cache node_modules or package managers',
        ...
        },
        {
            ...
            suggestion: 'Use `npx playwright install --with-deps` and/or run tests under `xvfb-run` on Linux runners',
