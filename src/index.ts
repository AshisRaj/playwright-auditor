...
export type Category = {
  ...
  findings: Finding[];
  score?: number;
  _maxPoints?: number; // internal: weight for overall score
};
...
type AnalyzerEntry = {
  id: string;
  title: string;
  stem: string; // filename stem (no extension), e.g. "locators"
  fnName: string; // exported function name
};
...
export type RunOptions = {
  outDir?: string;
  writeHtml?: boolean; // default true
  writeJson?: boolean; // default false
};

/** Utility: coerce to Sev */
function toSev(x: any): Sev {
...
function normalizeCategory(entry: AnalyzerEntry, out: any): Category | null {
  if (!out) return null;
...
function analyzerCandidates(stem: string): string[] {
  ...
  // 1) dist/analyzers/*
  for (const ext of exts) list.push(path.join(distAnalyzers, stem + ext));
  // 2) src/analyzers/* (ts-node or when running from src)
  for (const ext of exts) list.push(path.join(srcAnalyzers, stem + ext));
  // 3) same folder heuristic (rare but helpful)
  for (const ext of exts) list.push(path.join(dir, 'analyzers', stem + ext));

  return list;
}

/** Try to import analyzer module from many candidate paths */
async function importAnalyzerModule(
  stem: string,
): Promise<{ mod: any | null; tried: string[]; errors: string[] }> {
  const tried: string[] = [];
  const errors: string[] = [];
  ...
  for (const abs of candidates) {
    ...
    try {
      ...
      if (mod) return { mod, tried, errors };
    } catch (e) {
      errors.push(`${abs}: ${(e as any)?.message || String(e)}`);
    }
  }
  ...
  for (const ext of ['.js', '.mjs']) {
    ...
    try {
      ...
      errors.push(`${rel}: ${(e as any)?.message || String(e)}`);
    }
  }
...
async function runAnalyzer(
  ...
  try {
    let fn: any = mod[entry.fnName];
    if (typeof fn !== 'function') {
      // fallback: first exported function
      fn = Object.values(mod).find((v: any) => typeof v === 'function');
    }
    if (typeof fn !== 'function') {
      return {
        cat: null,
...
/** Programmatic API */
export async function runAudit(targetDir: string, options: RunOptions = {}): Promise<AuditResult> {
  ...
  const overallScore = computeOverallScore(
    cats.map((c) => ({ score: c.score || 0, _maxPoints: c._maxPoints || 0 })),
  );
...
async function mainFromCli() {
  ...
  const getFlag = (name: string, short?: string) =>
    argv.findIndex((a) => a === name || (short && a === short)) >= 0;
  const getValue = (name: string, short?: string) => {
  ...
  try {
    ...
    if (!skipHtml) {
      console.log(
        `✅ Audit complete. See ${path.join(outAbs, 'index.html')}${writeJson ? ' and report.json' : ''}`,
      );
    } else if (writeJson) {
    ...
    console.error('✖ Audit failed:', (e as any)?.message || String(e));
    process.exitCode = 1;
  }
}
