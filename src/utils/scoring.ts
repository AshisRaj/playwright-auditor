...
export interface CategoryScoreBreakdown {
  score: number; // 0..100
  maxPoints: number; // sum of weights of all findings in category
  earnedPoints: number; // sum of weights of passed findings
}
 ...
 */
export function computeCategoryScore(findings: FindingLike[]): CategoryScoreBreakdown {
  ...
  for (const f of findings) {
    const sev = (f?.severity ?? 'low') as Sev;
    const w = WEIGHT[sev] ?? WEIGHT.low;
    maxPoints += w;
    const passed = f?.status === 'pass';
    if (passed) earnedPoints += w;
  }
...
export function computeOverallScore(
  categories: Array<{ score: number; _maxPoints?: number }>,
): number {
