...
export async function analyzeNetwork(targetDir: string): Promise<CategoryResult> {
  ...
  // Evidence buckets
  const pageRouteFiles = new Set<string>(); // page.route(
  const contextRouteFiles = new Set<string>(); // context.route(
  const routeFulfillFiles = new Set<string>(); // route.fulfill({
  const routeAbortFiles = new Set<string>(); // route.abort()
  const routeContinueFiles = new Set<string>(); // route.continue(
  const routeRouteFiles = new Set<string>(); // route.route( (chained routing)
  const routeFromHarFiles = new Set<string>(); // routeFromHAR(
  const onRequestFiles = new Set<string>(); // .on('request', ...)
  ...
  for (const f of take) {
    ...
    if (rePageRoute.test(s)) pageRouteFiles.add(f);
    if (reContextRoute.test(s)) contextRouteFiles.add(f);
    if (reRouteFulfill.test(s)) routeFulfillFiles.add(f);
    if (reRouteAbort.test(s)) routeAbortFiles.add(f);
    if (reRouteContinue.test(s)) routeContinueFiles.add(f);
    if (reRouteRoute.test(s)) routeRouteFiles.add(f);
    if (reRouteFromHAR.test(s)) routeFromHarFiles.add(f);
    if (reOnRequest.test(s)) onRequestFiles.add(f);
  }
  ...
  const anyAction =
    routeFulfillFiles.size > 0 || routeAbortFiles.size > 0 || routeContinueFiles.size > 0;
  ...
  // 2) route.fulfill present
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Prefer await route.fulfill({ status, contentType, body }) for deterministic mocks. Consider storing payloads as fixtures.',
  ...
  // 3) route.abort present
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Use await route.abort() to simulate network errors/timeouts and test resilience paths.',
  ...
  // 4) route.continue present (pass-through with inspection)
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Use await route.continue() when only modifying headers/method/url before allowing the request.',
  ...
  // 5) route.route (chained/stacked routing)
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Chaining route.route() can lead to complex, hard-to-debug flows. Prefer a single interception per request.',
  ...
  // 6) routeFromHAR usage (HAR-based mocks)
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Use routeFromHAR for realistic recordings of network traffic and faster, stable tests when the backend is volatile.',
  ...
  // 7) Prefer page.route over context.route for test isolation
  {
    ...
    addFinding(cat, {
      ...
      severity: 'low',
      message:
        contextRouteFiles.size === 0
          ? 'No browserContext.route usage detected (good for isolation)'
          : `browserContext.route used in ${contextRouteFiles.size} file(s)`,
      suggestion:
        'Use page.route where possible to scope mocks to a single test. If using context.route, ensure proper setup/teardown.',
  ...
  // 8) Request event handling present
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Use request events for diagnostics (logging, assertions). Prefer route() for deterministic mocking.',
  ...
  // 9) Sanity: route present but no fulfill/abort/continue detected
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Every interception should either fulfill, abort, or continue the request; otherwise it can hang or behave unexpectedly.',
      artifacts: pick(new Set([...pageRouteFiles, ...contextRouteFiles])),
    });
  }
