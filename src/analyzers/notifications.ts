...
function hasEmailEvidence(text: string): boolean {
  ...
  if (
    /(SMTP_HOST|SMTP_SERVER|SMTP_USER|SMTP_PASS|SMTP_PORT)/i.test(text) &&
 ...
 */
function hasSlackEvidence(text: string): boolean {
  if (!text) return false;

  // Incoming webhook URL (most common)
  if (/https?:\/\/hooks\.slack\.com\/services\/[A-Z0-9/]+/i.test(text)) return true;
  ...
  if (/slack\.com\/api\//i.test(text) && /(fetch|axios|got)\s*\(/.test(text)) return true;
 ...
 */
function hasWebhookEvidence(text: string): boolean {
  if (!text) return false;

  const hasHttpPost =
    /(axios\.(post|request)\s*\(|fetch\s*\(|got\.\w+\s*\()/.test(text) &&
...
export async function analyzeNotifications(targetDir: string): Promise<CategoryResult> {
  ...
  const pkg = await readJsonSafe<PkgJson>(path.join(targetDir, 'package.json'));
  ...
  for (const f of files) {
    ...
    if (!slack && hasSlackEvidence(txt)) slack = true;
    if (!webhook && hasWebhookEvidence(txt)) webhook = true;
    if (!email && hasEmailEvidence(txt)) email = true;
    if (slack && webhook && email) break;
  }
  ...
  const checks: Array<[string, boolean, Sev, string, string]> = [
    [
      'Slack notifications',
      slack,
      'info',
