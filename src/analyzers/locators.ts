...
export async function analyzeLocators(targetDir: string): Promise<CategoryResult> {
  ...
  // Evidence buckets
  const xpathFiles = new Set<string>();
  const roleFiles = new Set<string>();
  const roleWithNameFiles = new Set<string>();
  const testIdFiles = new Set<string>();
  const userFacingGetByFiles = new Set<string>();
  const nthFiles = new Set<string>();
  const regexTextFiles = new Set<string>();
  const hasTextPseudoFiles = new Set<string>();
  const cssNthChildFiles = new Set<string>();
  const dataTestIdAttrFiles = new Set<string>();

  // Regexes
  const reXPath =
    /(locator\(\s*["'`](?:\/\/|\.\/\/)|page\.(?:locator|getByRole|getByTestId|getByText)?\(\s*["'`](?:\/\/|\.\/\/))/;
  ...
  for (const f of take) {
    ...
    if (reXPath.test(s)) xpathFiles.add(f);
    if (reGetByRole.test(s)) roleFiles.add(f);
    if (reGetByRoleWithName.test(s)) roleWithNameFiles.add(f);
    if (reGetByTestId.test(s) || reAttrDataTestId.test(s)) testIdFiles.add(f);
    if (reUserFacing.test(s)) userFacingGetByFiles.add(f);
    if (reNth.test(s)) nthFiles.add(f);
    if (reRegexText.test(s)) regexTextFiles.add(f);
    if (reHasTextPseudo.test(s)) hasTextPseudoFiles.add(f);
    if (reCssNthChild.test(s)) cssNthChildFiles.add(f);
    if (reDataTestIdAttrLiteral.test(s)) dataTestIdAttrFiles.add(f);
  }
  ...
  // 1) Avoid XPath selectors
  {
    ...
    addFinding(cat, {
      ...
      status: ok as unknown as 'pass' | 'fail',
      severity: 'high',
  ...
  // 3) When using getByRole, ensure accessible name is specified
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'When using getByRole, pass an explicit { name: /text/ } to target the intended element.',
  ...
  // 4) Use test IDs (getByTestId or [data-testid=...])
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Introduce data-testid attributes for key elements and use getByTestId or [data-testid=...] selectors.',
  ...
  // 6) Use user-facing getBy* variants
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Prefer getByText/Label/Placeholder/AltText/Title where appropriate to align with how users interact.',
  ...
  // 8) Avoid regex getByText
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Prefer exact strings or accessible role+name. Regex text selectors can be brittle and slow.',
  ...
  // 9) Avoid :has-text() pseudo selector
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Use getByText / getByRole({ name }) / hasText option on locator() instead of :has-text().',
  ...
  // 10) Avoid CSS :nth-child()
  {
    ...
    addFinding(cat, {
      ...
      suggestion:
        'Prefer semantic locators (role/name or test IDs) rather than positional CSS like :nth-child().',
