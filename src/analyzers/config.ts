...
import { applyScore, createCategory, findConfig, ok, readFile, Sev } from '../utils/helpers.js';

const cat = createCategory('config', 'Config');

export async function analyzeConfig(targetDir: string): Promise<CategoryResult> {
  ...
  const rules: Array<{
    ...
    },
    {
      ...
      suggestion:
        'Add `reporter: [["list"], ["html", { open: "never" }]]` to persist interactive reports',
    },
    {
      id: 'reporter-junit',
      ...
      suggestion:
        'Consider adding `["junit", { outputFile: "test-results/junit.xml" }]` for CI insights',
    },
    {
      id: 'baseurl',
    ...
    },
    {
      ...
      suggestion:
        'Set `expect: { timeout: 10000 }` to control default assertion timeout per expect()',
    },
    {
      id: 'timeout-sane',
      ...
      suggestion:
        'Prefer explicit waits/assertions and keep per-test timeout ≤ 60s; rely on expect timeouts',
    },
    {
      id: 'outputdir',
      ...
      suggestion:
        'Set `outputDir: "test-results/"` to keep results organized for CI artifact upload',
    },
    {
      id: 'webserver',
      ...
      suggestion:
        'Configure `webServer` to start your app for E2E and set `reuseExistingServer: true` to speed up local runs',
    },
  ];
