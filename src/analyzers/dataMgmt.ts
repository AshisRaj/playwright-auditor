...
async function safeReadFile(p?: string | null): Promise<string> {
  if (!p) return '';
  try {
...
export async function analyzeDataMgmt(targetDir: string): Promise<CategoryResult> {
  // Common paths & files
  ...
  // Heuristics / regexes
  const dotenvInConfig =
    /dotenv|config\(\)/i.test(pwConfigText) || /process\.env\./.test(pwConfigText);
  const globalSetupConfigured =
    /global-setup/i.test(pwConfigText) || /globalSetup/i.test(pwConfigText);
  const globalTeardownConfigured =
    /global-teardown/i.test(pwConfigText) || /globalTeardown/i.test(pwConfigText);

  const storageStateConfigured =
    /storageState\s*:\s*['"`][^'"`]+['"`]/.test(pwConfigText) ||
    /use\s*:\s*{[^}]*storageState/i.test(pwConfigText);

  const outputDirConfigured = /outputDir\s*:\s*['"`][^'"`]+['"`]/.test(pwConfigText);
  const screenshotsConfigured =
    /screenshot\s*:\s*['"`](on|only-on-failure)['"`]|screenshot\s*:\s*(true|false)/i.test(
      pwConfigText,
    );
  const tracesConfigured = /trace\s*:\s*['"`](on|off|retain-on-failure|on-first-retry)['"`]/i.test(
    pwConfigText,
  );
  const videoConfigured = /video\s*:\s*['"`](on|off|retain-on-failure)['"`]/i.test(pwConfigText);

  const suspectedSecretsRegexes: RegExp[] = [
  ...
  const hasCleanScript =
    !!pkg?.scripts?.clean &&
    /(rimraf|rm\s+-rf|del-cli).*(test-results|trace|screenshots|downloads)/i.test(
      pkg.scripts.clean,
    );
  ...
  const hasSuspectedSecrets = probableLeakTexts.some((t) =>
    suspectedSecretsRegexes.some((re) => re.test(t)),
  );
  ...
  addFinding({
    ...
    suggestion:
      'Remove hardcoded secrets. Use CI secrets or .env. Consider adding a pre-commit secret scanner (e.g., gitleaks, trufflehog).',
    artifacts: probableLeakFiles,
  });

  // 3) Artifacts: directories & config
  const artifactsPresent =
    (await exists(results)) ||
  ...
  addFinding({
    ...
    sev: 'info',
    suggestion:
      'Ensure CI preserves artifacts for debugging (test-results, trace, screenshots, downloads).',
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Configure outputDir, screenshot, trace, and video in playwright.config.* to standardize artifact locations.',
    artifacts: [pwConfigPath || ''],
  });
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'In GitHub Actions, use actions/upload-artifact to persist test-results/trace/screenshots on failures.',
  ...
  const ignoredArtifacts =
    hasGitignore && /(test-results|trace|screenshots|downloads)/.test(gitignoreText);
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Add test-results/, trace/, screenshots/, downloads/ to .gitignore to keep repo clean.',
    artifacts: [gitignore],
  });
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Add global-setup.ts / global-teardown.ts for seeding/cleanup/session bootstrapping if needed.',
    artifacts: [
      'global-setup.ts',
      'global-teardown.ts',
      ...globalSetupFiles,
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Use storageState to reuse auth sessions and avoid re-login. Store files under ./.auth and reference in use.storageState.',
  ...
  addFinding({
    ...
    sev: 'info',
    suggestion:
      'Prefer a dedicated .auth folder for session artifacts with proper .gitignore rules.',
    artifacts: [authDir],
  });
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Maintain test data as JSON/CSV/XLSX in a dedicated folder (e.g., test-data/ or __fixtures__/).',
  ...
  addFinding({
    ...
    sev: 'info',
    suggestion:
      'Use test-data/, __fixtures__/, or resources/ to separate data from tests and code.',
  ...
  addFinding({
    ...
    sev: 'low',
    suggestion:
      'Validate required env vars on startup with zod/envalid/joi/yup to fail fast on misconfigurations.',
  ...
  addFinding({
    ...
    sev: 'info',
    suggestion:
      'Add "clean" script (rimraf test-results trace screenshots downloads) and run before/after CI jobs as needed.',
    artifacts: [pkgJsonPath],
  });
