...
export async function analyzeCore(targetDir: string): Promise<CategoryResult> {
  ...
  const testRoots = ['tests', 'test', 'e2e', '__tests__', 'src'].map((r) =>
  ...
  const hasPagesDir =
    (await exists(path.join(targetDir, 'src', 'pages'))) ||
  ...
  const hasUtilsDir =
    (await exists(path.join(targetDir, 'src', 'utils'))) ||
  ...
  for (const f of testFiles.slice(0, 1200)) {
    ...
    if (/\bexport\s+class\s+\w+Page\b/.test(t) || /\bclass\s+\w+Page\b/.test(t)) {
      ...
      if (pageObjectHits.length >= 5) break;
    }
  }

  // --- Environment switching heuristics ---
  const hasDotenvFiles =
    (await exists(path.join(targetDir, '.env'))) ||
  ...
  const tagInTitleRe = new RegExp(
    `(?:test|it|describe)\\s*\\(\\s*(['"\`])[\\s\\S]*?@${tagWord}[\\s\\S]*?\\1`,
    'i',
  );

  const filesWithTags: string[] = [];
  for (const f of testFiles.slice(0, 3000)) {
    const t = await readFile(f);
    if (tagInTitleRe.test(t)) {
      filesWithTags.push(f);
      if (filesWithTags.length >= 12) break; // limit artifacts
    }
  }
  ...
  const retriesPass =
    /\bretries\s*:\s*(?!0\b)(\d+|process\.env\.[A-Za-z_]\w*\s*\?\s*\d+\s*:\s*\d+)/i.test(cfgText);
  const headlessOrWorkersPass =
    /\bheadless\s*:\s*true\b/i.test(cfgText) ||
  ...
  const checks: Array<{
    ...
    },
    {
      ...
      suggestion:
        'Factor common flows into Page Object classes (`src/pages/`) and utilities (`src/utils/`).',
    ...
    },
    {
      ...
      suggestion:
        'Provide env switching via dotenv/.env and `use.baseURL` or process.env variables in config.',
      file: cfgPath || '',
    },
    {
      id: 'grep-config',
    ...
    },
    {
      ...
      suggestion:
        'Add scripts like `"test:smoke": "playwright test --grep @smoke"` in package.json.',
      file: pkgPath,
    ...
    },
    {
      ...
      suggestion:
        'Append tags in titles, e.g., `test("login @smoke", ...)` or `describe("checkout @regression", ...)`.',
